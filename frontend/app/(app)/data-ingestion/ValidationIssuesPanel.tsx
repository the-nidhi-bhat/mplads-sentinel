...
export default function ValidationIssuesPanel({ rows }: { rows: ValidatedRow[] }) {
  const flagged = rows.filter((r) => r._validationStatus !== "valid");
  const [open, setOpen] = useState(flagged.some((r) => r._validationStatus === "rejected"));
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between px-4 py-3 text-sm font-semibold text-[var(--text-primary)]"
      >
            ...
            <div key={row._rowNumber} className="px-4 py-2.5 flex items-start gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-xs font-semibold text-slate-700">
                  Row {row._rowNumber} · {row.project_id || "Unknown ID"}
                </p>
